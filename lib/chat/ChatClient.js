...
class ChatClient extends EventEmitter {
  ...
  onChat(text, callback) {
    if (!this.room)
      throw new Error('Please join a room first.')
  ...
  // ** TODO: Prevent multiple connects from one user
  onLogin(email, password, nickname, callback) {
    return this.users.login(email, password, nickname).then(() => {
  ...
  onDisconnect(callback) {
    this.users.logout(this.nickname);
    this.emit('left', this.nickname, this.room);
